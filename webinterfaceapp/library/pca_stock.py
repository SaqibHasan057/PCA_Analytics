...
class PcaRealTime:
    def robust_real_time_PCA_target_dimension(self, input_data, block_size, output_dimension):
        ...
        for i in range(block_size, input_data.shape[0]):
            ...
            # top eigenvector
            reconstruction_error = np.matmul(block,
                                             identity_matrix - np.matmul(eigen_vectors, np.transpose(eigen_vectors)))
            pca = decomposition.PCA(n_components=output_dimension)
            pca.fit(reconstruction_error)
            ...
            if (max_eigen_value > min_eigen_value):
                ...
                eigen_values = np.delete(eigen_values, (minIndex), axis=0)
                eigen_vectors = np.delete(eigen_vectors, (minIndex), axis=0)
                eigen_values = np.append(eigen_values, max_eigen_value)
                eigen_vectors = np.append(eigen_vectors, np.transpose(max_eigen_vector), axis=0)
                # }
            ...
            # RANAK{
            abs_eigen_vectors = np.absolute(eigen_vectors)
            max_dims = abs_eigen_vectors.argmax(axis=0)
            max_eigen_values_index = eigen_values.argmax(axis=0)
            max_dim = max_dims[max_eigen_values_index]
            dim_array.append(max_dim + 1)
            # }

        output_matrix = np.squeeze(np.array(output_matrix))
        return output_matrix, eigen_vectors, eigen_values, dim_array
